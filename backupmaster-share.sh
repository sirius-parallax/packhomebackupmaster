...
NETWORK_MOUNT_POINT="/mnt/network_backup"  # Точка монтирования для сетевой папки
...
check_and_install_package "mount"  # Для монтирования сетевых папок
check_and_install_package "cifs-utils"  # Для Samba (CIFS)
...
check_and_kill_open_files() {
    ...
    if [ -n "$OPEN_FILES" ]; then
        ...
        read -p "Убить процессы, использующие эти файлы? (y/n/q для возврата): " KILL_PROCESSES
        if [ "$KILL_PROCESSES" = "q" ]; then
            echo "Возврат в предыдущее меню."
            return 1
        fi
...
# Функция для бэкапа пользователей на SSH-сервер
backup_users_ssh() {
...
# Функция для бэкапа пользователей на сетевую папку
backup_users_network() {
    echo -e "${GREEN}Настройка подключения к сетевой папке...${NC}"
    read -p "Введите IP-адрес или имя сервера (например, 192.168.1.100): " SERVER
    if [ -z "$SERVER" ]; then
        echo -e "${RED}Адрес сервера не указан${NC}"
        return
    fi
    read -p "Введите путь к сетевой папке (например, /share/backups): " SHARE_PATH
    if [ -z "$SHARE_PATH" ]; then
        echo -e "${RED}Путь к сетевой папке не указан${NC}"
        return
    fi
    read -p "Введите логин (оставьте пустым, если не требуется): " LOGIN
    read -s -p "Введите пароль (оставьте пустым, если не требуется): " PASSWORD
    echo

    # Создание точки монтирования
    if [ ! -d "$NETWORK_MOUNT_POINT" ]; then
        sudo mkdir -p "$NETWORK_MOUNT_POINT"
        if [ $? -ne 0 ]; then
            echo -e "${RED}Не удалось создать точку монтирования $NETWORK_MOUNT_POINT${NC}"
            return
        fi
    fi

    # Монтирование сетевой папки
    echo -e "${GREEN}Монтирование сетевой папки...${NC}"
    if [ -n "$LOGIN" ] && [ -n "$PASSWORD" ]; then
        sudo mount -t cifs "//$SERVER$SHARE_PATH" "$NETWORK_MOUNT_POINT" -o username="$LOGIN",password="$PASSWORD" 2>mount_errors.log
    else
        sudo mount -t cifs "//$SERVER$SHARE_PATH" "$NETWORK_MOUNT_POINT" -o guest 2>mount_errors.log
    fi
    if [ $? -ne 0 ]; then
        echo -e "${RED}Не удалось подключиться к сетевой папке $SERVER$SHARE_PATH:${NC}"
        cat mount_errors.log
        rm -f mount_errors.log
        return
    fi
    rm -f mount_errors.log
    ...
    if [ ${#USERS[@]} -eq 0 ]; then
        echo -e "${RED}Пользователи с UID >= 1000 и директориями в /home не найдены${NC}"
        sudo umount "$NETWORK_MOUNT_POINT"
        return
    fi
    ...
    echo "all. Забэкапить всех"
    echo "q. Вернуться в подменю"
    read -p "Выберите номер пользователя, 'all' или 'q': " USER_CHOICE

    if [ "$USER_CHOICE" = "q" ]; then
        echo "Возврат в подменю."
        sudo umount "$NETWORK_MOUNT_POINT"
        return
    fi
    ...
    for user in "${SELECTED_USERS[@]}"; do
        ...
        echo -e "${GREEN}Бэкап пользователя $user...${NC}"
        echo "Примерный размер бэкапа: $BACKUP_SIZE МБ"
        USER_BACKUP="$NETWORK_MOUNT_POINT/home_backup_${user}_$TIMESTAMP.tar.gz"
        sudo tar -czf "$USER_BACKUP" "$USER_HOME" 2>tar_errors.log
        if [ $? -eq 0 ]; then
            echo "Домашняя директория $user забэкаплена: $USER_BACKUP"
            rm -f tar_errors.log
    ...
    # Размонтирование сетевой папки
    sudo umount "$NETWORK_MOUNT_POINT"
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}Сетевая папка успешно размонтирована${NC}"
    else
        echo -e "${RED}Ошибка при размонтировании $NETWORK_MOUNT_POINT${NC}"
    fi
}

# Функция для восстановления бэкапов с сетевой папки
restore_users_network() {
    echo -e "${GREEN}Настройка подключения к сетевой папке...${NC}"
    read -p "Введите IP-адрес или имя сервера (например, 192.168.1.100): " SERVER
    if [ -z "$SERVER" ]; then
        echo -e "${RED}Адрес сервера не указан${NC}"
    ...
    USER_BACKUPS=("$NETWORK_MOUNT_POINT"/home_backup_*.tar.gz)

    echo -e "${GREEN}Поиск бэкапов в $NETWORK_MOUNT_POINT...${NC}"
    ...
    if [ ${#USER_BACKUPS[@]} -eq 0 ] || [ ! -e "${USER_BACKUPS[0]}" ]; then
        echo -e "${RED}Бэкапы не найдены в $NETWORK_MOUNT_POINT${NC}"
        sudo umount "$NETWORK_MOUNT_POINT"
        return
    fi
    ...
    for backup in "${SELECTED_BACKUPS[@]}"; do
        BACKUP_SIZE=$(ls -l "$backup" | awk '{print int($5 / 1024 / 1024)}')
        ...
        if [[ "$backup" =~ home_backup_(.+)_[0-9]{8}_[0-9]{6}\.tar\.gz ]]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                if ! id "$USER" >/dev/null 2>&1; then
                    ...
                    read -p "Создать пользователя $USER? (y/n/q для возврата): " CREATE_USER
                    ...
                    if [ "$CREATE_USER" = "y" ]; then
                        ...
                        if [ $? -eq 0 ]; then
                            ...
                            read -p "Установить пароль для $USER? (y/n/q для возврата): " SET_PASSWD
...
# Функция для восстановления бэкапов с SSH-сервера
restore_users_ssh() {
    echo -e "${GREEN}Настройка подключения к удалённому серверу...${NC}"
    read -p "Введите имя сервера (например, example.com): " SERVER
    if [ -z "$SERVER" ]; then
    ...
    for backup in "${SELECTED_BACKUPS[@]}"; do
        ...
        if [ $? -eq 0 ]; then
            BACKUP_SIZE=$(ls -l "$TEMP_BACKUP" | awk '{print int($5 / 1024 / 1024)}')
            echo "Размер бэкапа $backup: $BACKUP_SIZE МБ"
            if [ "$BACKUP_SIZE" -gt "$FREE_SPACE" ]; then
                echo -e "${RED}Недостаточно свободного места для восстановления $backup (требуется $BACKUP_SIZE МБ, доступно $FREE_SPACE МБ)${NC}"
                sudo rm -f "$TEMP_BACKUP"
...
# Подменю для бэкапов на сетевую папку
network_backup_menu() {
    while true; do
        clear
        echo -e "${GREEN}=== Подменю бэкапов на сетевую папку ===${NC}"
        echo "1. Создать бэкап на сетевую папку"
        echo "2. Восстановить бэкап с сетевой папки"
...
while true; do
    ...
    echo "3. Создать/восстановить бэкап на сетевой папке"
    echo "4. Восстановить локальный бэкап пользователей"
    echo "5. Восстановить бэкап пользователей с SSH-сервера"
    echo "6. Удалить локальные бэкапы"
    echo "7. Выйти"
